...
from torchreid.data_manager import ImageDataManager, BothImageVideoDatamanager
...
def main():
    global args
    glob_min_loss = 0.0021
    model_loss = {'htri'}
    args.source_names = ['market1501', 'cuhk03', 'dukemtmcreid', 'msmt17',
                         'cuhk01', 'grid', 'viper', 'prid450s', 'ilidsvid', 'prid2011', 'mars']
    # args.source_names = ['mars']
    args.target_names = ['market1501']
    args.htri_only = True
    ...
    args.gamma = 0.1
    args.max_epoch = 100
    # args.stepsize = [80, 140, 220, 280, 340, 380]
    args.stepsize = [28, 67,80, 160]
    args.train_batch_size = 164
    args.num_instances = 4
    args.test_batch_size = 100
    args.arch = 'se_resnet50_fc512'
    args.save_dir = 'log/se_resnet50_fc512-11'
    args.gpu_devices = '0'
    args.lambda_xent = 1
    args.lambda_htri = 1
    # args.evaluate = True
    # args.fixbase_epoch = 10
    # args.open_layers = ['classifier']
    args.resume = 'log/se_resnet50_fc512-11/quick_save_checkpoint_ep67.pth.tar'
    # 29-8data + prid450s + ilids -grid - viper
    # args.load_weights = 'log/se_resnet50_fc512-11/quick_save_checkpoint_ep47.pth.tar'
    # args.start_epoch = 47
    ...
    dm = BothImageVideoDatamanager(use_gpu, **image_dataset_kwargs(args))
    trainloader, testloader_dict = dm.return_dataloaders()

    print("Initializing model: {}".format(args.arch))
    model = models.init_model(
        name=args.arch, num_classes=dm.num_train_pids, loss=model_loss)
    ...
    if args.evaluate:
        ...
        for name in args.target_names:
            ...
            distmat = test(model, queryloader, galleryloader,
    ...
    if args.fixbase_epoch > 0:
        ...
        for epoch in range(args.fixbase_epoch):
            ...
            train(epoch, model, criterion_xent, criterion_htri,
                  optimizer, trainloader, use_gpu, fixbase=True)
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        start_train_time = time.time()
        local_loss = train(epoch, model, criterion_xent, criterion_htri,
...
def train(epoch, model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu, fixbase=False):
    ...
    for batch_idx, (imgs, pids, _, _) in enumerate(trainloader):
        ...
        # with torch.no_grad():
        #     features = model(imgs)
        # print('Time no grad: ',time.time()-s,'s')
        ...
        if args.htri_only:
            ...
            outputs, features = model(imgs)
            # max_dist = print_max_dist(features.clone(),pids.clone())
            # print(max_dist)
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            ...
            # print('Xent loss: ', xent_loss.item(),
            #       'Htri loss: ', htri_loss.item())

        end = time.time()
    return losses.avg


def print_max_dist(feature, pids):
    max_dist = 0
    from scipy.spatial.distance import cdist
    for i in range(len(pids)):
        for j in range(len(pids)):
            if i != j and pids[i] == pids[j]:
                dist = torch.sqrt(
                    torch.sum(torch.pow(feature[i]-feature[j], 2))).cpu().data
                if dist > max_dist:
                    max_dist = dist
    # print('aaa',max_dist)
    return max_dist
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20], return_distmat=False):
    ...
    cmc, mAP = evaluate(distmat, q_pids, g_pids, q_camids,
                        g_camids, use_metric_cuhk03=args.use_metric_cuhk03)
