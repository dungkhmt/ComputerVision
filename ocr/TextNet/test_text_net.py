...
def process_output(image, output, raw_img, threshold=0.5):
    ...
    write_image_result('./trash/parent.png', np.abs(parent[:, :, 0]*parent[:, :, 1]))
    ...
    seg2bbox(res, cv2.resize(raw_img, (width, height)), raw_img)
    # cv2.imshow('1', res)
    # cv2.waitKey(0)
...
def seg2bbox(seg, img_resize, raw_img):
    ...
    for idx in range(int(np.amax(seg))):
        ...
        cv2.drawContours(raw_img, [cnt], 0, colors[idx], 3)
    ...
    list_result = glob.glob('./result/*.jpg')
...
def test(path_image):
    ...
    transform = build_transforms(
        maxHeight=354, maxWidth=354, is_train=False)
    # im = torch.Tensor(im)
    img, _ = transform(np.copy(image), None)
    im = img.transpose(2, 0, 1)
    im = torch.Tensor(im).to('cuda').unsqueeze(0)
    model = init_model(name='se_resnext50_32x4d')
    load_model(
        model, 'log/se_resnext101_32x4d-final-text-net-synth-text/quick_save_checkpoint_ep1_100000.pth.tar')
    model = model.to('cuda')
    import time
    # while True:
    start = time.time()
    ...
    with torch.no_grad():
        output = model(im)
    process_output(img, output[0].to('cpu').numpy(), image)
    print('Time: ', time.time()-start, ' s')


if __name__ == "__main__":
    # test('./data/pdf-text/Images/Train/img1768.jpg')
    test('./img_test/ant+hill_1_0.jpg')
