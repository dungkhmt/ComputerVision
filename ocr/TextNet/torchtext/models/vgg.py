...
class VGG(nn.Module):
    ...
    def __init__(self, features, init_weights=True):
        ...
        self._construct_stage_layer()

        if init_weights:
            self._initialize_weights()

    def _construct_stage_layer(self):
        index_stage_layer =[0]
        for i,m in enumerate(self.features.children()):
            if isinstance(m, nn.MaxPool2d):
                index_stage_layer.append(i+1)
                
        assert len(index_stage_layer)==6

        self.stage1 = nn.Sequential(*[self.features[layer] for layer in range(index_stage_layer[0], index_stage_layer[1])])
        self.stage2 = nn.Sequential(*[self.features[layer] for layer in range(index_stage_layer[1], index_stage_layer[2])])
