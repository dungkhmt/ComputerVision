...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
        ...
        self.max_pool2d = nn.MaxPool2d(3, stride=2, ceil_mode=True)
...
def se_resnet50_fc512(pretrained=False, **kwargs):
    model = SENet(
        block=SEResNetBottleneck,
        layers=[3, 4, 6, 3],
        groups=1,
...
if __name__ == "__main__":
    ...
    model = se_resnext101_32x4d().to('cuda')
    C1, C2, C3, C4, C5 = model(input)
    import torchsummary
    print(torchsummary.summary(model,(3,512,512),batch_size=1))
    print(C1.size())
    print(C2.size())
    print(C3.size())
    print(C4.size())
